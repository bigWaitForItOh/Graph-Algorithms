###########################################################################################################################
#IMPLEMENTATION OF Dijkstra's Shortest Path Algorithm for weighted, directed graphs. Yeilds shortest path LENGTHS from
#source to every other vertex (needs to be modified to include path information).
###########################################################################################################################

#takes the graph (described below) and the source vertex as arguments and returns a dictionary which contains all the vertices and their corresponding shortest path lengths. Returns -1 if the Vertex is unreachable.
...
for i in dists: print (i, dists [i]);
